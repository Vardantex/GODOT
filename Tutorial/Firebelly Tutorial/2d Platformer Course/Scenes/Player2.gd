...
#Make character jump properly
	...
	if (velocity.y < 0 and not Input.is_action_pressed("ARROW_UP")):
	...
	#Check to DASH2 only on floor
	if (is_on_floor()):
		hasDash = true
	
	if (hasDash && Input.is_action_just_pressed("DASH2")):
		call_deferred("change_state", State.DASHING)
		hasDash = false 

#This custom process function will handle all the movement for dashing
func _process_dash(delta):
	#Apply the DASH2 modifier when state is new
...
func get_movement_vector():
	...
	moveVector.x = Input.get_action_strength("ARROW_RIGHT") - Input.get_action_strength("ARROW_LEFT")
